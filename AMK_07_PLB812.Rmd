...
```{r}
penguins = read.table('/Users/abigailkrent/Desktop/penguins.txt', header=T)
print(penguins)
```
...
geom_histogram(binwidth=.5) #the type of plot we want to make
```
...
```{r}
ggplot(penguins, aes(x = flipper_length_mm)) + #information about the dataset
  geom_histogram(binwidth=.5, color = "white", fill="navy") + #make the figure
...
```{r}
penguins$body_mass_g <- penguins$body_mass_g/1000 #overwriting the mass column to be kg :)
names(penguins)[6] <- "body_mass_kg"
ggplot(penguins, aes(x=body_mass_kg, fill = species)) +
  geom_histogram(colour = "black") +
  xlab("Body Mass (kg)") +
  ylab("Frequency") +
...
ggplot(penguins, aes(x = flipper_length_mm, fill = species)) +
  ...
  ylab("Density") +
  ggtitle("Flipper Length by Penguin Species")
...
ggplot(penguins, aes(x = bill_depth_mm, y = flipper_length_mm, color = species)) +
  ...
  ggtitle("Bill Depth by Flipper Length Across Penguin Species")
...
```{r}
# I had to change scale_colour_manual to scale_fill_manual to get the code to run. Changing the fill to be body mass instead of species added another layer of depth to the graph that I don't know that I love... 

ggplot(penguins, aes(x = bill_depth_mm, y = flipper_length_mm, color = body_mass_kg)) +
  geom_point() +
  theme_minimal() +
  scale_fill_manual(values = c("pink", "orange", "blue")) + 
  geom_point(size = 2) +
